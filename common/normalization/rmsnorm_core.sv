module rmsnorm_core #(
  ...
  function automatic int unsigned rms_denominator(
    ...
    for (int lane = 0; lane < ByteCount; lane++) begin
      if (input_keep[lane]) begin
        ...
        square_sum += input_lane * input_lane;
        active_count++;
      end
    end
    ...
    if (active_count == 0) begin
      ...
      mean_square = (square_sum / active_count) + EPSILON;
      root = isqrt(mean_square);
      rms_denominator = (root == 0) ? 1 : root;
    end
  endfunction
  ...
  function automatic logic [DATA_W-1:0] apply_rmsnorm(
    ...
    result = '0;
    denominator = rms_denominator(input_data, input_keep & weight_keep);
    for (int lane = 0; lane < ByteCount; lane++) begin
      if (input_keep[lane] && weight_keep[lane]) begin
        input_lane = $signed(input_data[(lane * 8) +: 8]);
        weight_lane = $signed(weight_data[(lane * 8) +: 8]);
        scaled_lane = (input_lane * weight_lane) / int'(denominator);
