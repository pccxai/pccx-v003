module gelu_unit #(
  ...
  function automatic int unsigned gelu_gate(input int signed value);
    if (value <= -64) begin
      gelu_gate = 0;
    end else if (value >= 64) begin
      gelu_gate = 128;
    end else begin
      gelu_gate = value + 64;
    end
  ...
  function automatic logic [DATA_W-1:0] apply_gelu(
    ...
    for (int lane = 0; lane < ByteCount; lane++) begin
      if (input_keep[lane]) begin
        ...
        output_lane = (input_lane * int'(gelu_gate(input_lane))) >>> 7;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (input_fire) begin
        output_data_q  <= apply_gelu(S_INPUT.data, S_INPUT.keep);
